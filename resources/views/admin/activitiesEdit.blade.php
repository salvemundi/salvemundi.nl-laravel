            ...
            <form action="/admin/activities/edit/store" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}"
                        value="{{ $activities->name }}" id="name" name="name" placeholder="Naam...">
                </div>

                <div class="form-group">
                    <label for="link">Microsoft forms link</label>
                    <input class="form-control{{ $errors->has('link') ? ' is-invalid' : '' }}"
                        value="{{ $activities->formsLink }}" id="link" name="link" placeholder="Forms link...">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" min="0" step=".01"
                        ...
                        value="{{ $activities->amount }}" id="price" name="price" placeholder="Prijs...">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" min="0"
                        ...
                        value="{{ $activities->limit }}" id="limit" name="limit"
                        placeholder="Limiet aantal inschrijvingen...">
                </div>

                <div class="form-group">
                    <label for="Achternaam">Prijs voor niet leden*</label>
                    <input type="number" min="0" step=".01"
                        class="form-control{{ $errors->has('price2') ? ' is-invalid' : '' }}"
                        value="{{ $activities->amount_non_member }}" id="price2" name="price2" placeholder="Prijs...">
                </div>
                ...
                <div class="form-group mb-2">
                    ...
                    <select id="tags" name="tags[]" class="form-select" multiple aria-label="multiple select example">
                        @foreach ($tags as $tag)
                            ...
                            @endphp
                            <option @if ($selected) selected @endif value="{{ $tag->id }}">
                                {{ $tag->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea type="textarea" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" name="description"
                        placeholder="Beschrijving...">{{ $activities->description }}</textarea>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <input type="datetime-local" name="startDate" value="{{ $activities->startDate }}"
                            class="form-control" placeholder="Vanaf" aria-label="from">
                        <span class="input-group-text">Tot</span>
                        <input type="datetime-local" name="endDate" value="{{ $activities->endDate }}" class="form-control"
                            placeholder="Tot" aria-label="until">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea type="textarea" class="form-control{{ $errors->has('membersOnlyContent') ? ' is-invalid' : '' }}"
                        name="membersOnlyContent" placeholder="Beschrijving...">{{ $activities->membersOnlyContent }}</textarea>
                </div>
                ...
                <div class="form-group" id="ticketsPerRound">
                    ...
                    <input type="number" min="0"
                        class="form-control{{ $errors->has('maxTicketOrderAmount') ? ' is-invalid' : '' }}"
                        name="maxTicketOrderAmount" placeholder="Tickets per keer..."
                        value="{{ $activities->maxTicketOrderAmount }}" />
                </div>
                    ...
                    <div id="associationInputs">
                        @foreach ($activities->associations as $association)
                            <div class="d-flex" id="{{ $association->id }}">
                                <input type="text" name="associationName[]" class="form-control mb-2"
                                    value="{{ $association->name }}">
                                <a onclick="document.getElementById({{ $association->id }}).remove()"
                                    class="btn btn-primary mb-2"><i class="fas fa-minus fa-sm"></i></a>
                            </div>
                        @endforeach
                    </div>
                </div>
                    ...
                    <div class="input-group-prepend">
                        <div class="custom-file" style="width: 80px;">
                            ...
                            <input type="file" onchange="CopyMe(this, 'imgPath');" class="custom-file-input"
                                style="height: 0px;" id="photo" name="photo"
                                aria-describedby="inputGroupFileAddon01">
                        </div>
                    </div>
                    <div class="custom-file form-control">
                        <input style="border: hidden;" id="imgPath" name="imgPath" value="{{ $activities->imgPath }}"
                ...
                <label class="cbx" for="cbx"><span>
                        ...
                        </svg></span><span>Voor deze activiteit kan maar eenmaal ingeschreven worden per
                        deelnemer</span></label>

                @if ($activities->membersOnly)
                    <input class="inp-cbx" id="cbx2" name="cbxMembers" type="checkbox" checked
                        style="display: none" />
                @else
                ...
                @if ($activities->isGroupSignup)
                    <input class="inp-cbx" id="cbx3" name="cbxGroup" type="checkbox" checked
                        style="display: none" />
                @else
