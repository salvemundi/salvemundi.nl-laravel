    ...
    <div id="contact" class="col-auto col-md-6 col-sm-12">
        ...
        <form action="/admin/coupons/edit/{{$coupon->id}}" method="post">
            ...
            <div class="form-group">
                ...
                <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" value="{{ $coupon->name }}" id="name" name="name" placeholder="Naam...">
            </div>

           <div class="form-group">
                <label for="exampleFormControlTextarea1">Beschrijving</label>
                <textarea type="textarea" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" name="description" placeholder="Beschrijving...">{{{ $coupon->description }}}</textarea>
            </div>

            <div class="form-group">
                <label for="Achternaam">Korting in euros*</label>
                <input type="number" min="1" step=".01" class="form-control{{ $errors->has('price') ? ' is-invalid' : '' }}" value="{{ $coupon->value }}" id="price" name="price" placeholder="Prijs...">
            </div>

            <div class="form-group">
                <label for="valuta">Valuta*</label>
                <input value="EUR" class="form-control{{ $errors->has('valuta') ? ' is-invalid' : '' }}" value="{{ $coupon->currency }}" id="valuta" name="valuta" placeholder="Valuta...">
            </div>
