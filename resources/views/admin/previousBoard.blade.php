...
@section('title', 'Admin | Besturen – ' . config('app.name'))
@section('content')
    <script>
                    ...
                    <tbody>
                        @foreach ($previousBoard as $bestuur)
                            <tr id="tr-id-3" class="tr-class-2" data-title="bootstrap table">
                                ...
                                <td data-value="{{ $bestuur->id }}">
                                    <form method="post" action="/admin/oud-bestuur/delete">@csrf<input type="hidden"
                                            name="id" id="id" value="{{ $bestuur->id }}"><button type="submit"
            ...
            <form action="/admin/oud-bestuur/store" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control{{ $errors->has('year') ? ' is-invalid' : '' }}" value="{{ old('year') }}"
                        type="number" id="year" name="year" placeholder="Jaar...">
                </div>

                <div class="form-group">
                    <label for="reference">Bestuur*</label>
                    <textarea class="form-control{{ $errors->has('bestuur') ? ' is-invalid' : '' }}" value="{{ old('bestuur') }}"
                        type="text" id="bestuur" name="bestuur" placeholder="Bestuur..."></textarea>
                </div>
