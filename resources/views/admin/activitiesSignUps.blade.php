                    ...
                    <tbody>
                        @foreach ($users as $user)
                            <tr id="tr-id-3" class="tr-class-2" data-title="bootstrap table">
                                ...
                                <td data-value="{{ $user->birthday }}">{{ date('d-m-Y', strtotime($user->birthday)) }}
                                </td>
                                <td data-value="{{ $user->id }}">
                                    <form method="post"
                                        action="/admin/activiteiten/{{ $activity->id }}/remove/{{ $user->id }}">
                                        @csrf
                        ...
                        @foreach ($userTransactionInfo as $user)
                            <tr id="tr-id-3" class="tr-class-2" data-title="bootstrap table">
                                ...
                                <td class="tr-id-3{{ isset($user[4]) ? $user[4] : '' }}" data-value="{{ $user[1] }}">
                                    {{ $user[1] }} @isset($user[5])
                                        {{ ' - ' . $user[5] }}
                                    @endisset
                                </td>
                                <td class="tr-id-3{{ isset($user[4]) ? $user[4] : '' }}" data-value=""></td>
                                <td class="tr-id-3{{ isset($user[4]) ? $user[4] : '' }}" data-value="{{ $user[0] }}">
                                    {{ $user[0] }}</td>
                                <td class="tr-id-3{{ isset($user[4]) ? $user[4] : '' }}" data-value=""></td>
                                <td class="tr-id-3{{ isset($user[4]) ? $user[4] : '' }}" data-value=""></td>
                            </tr>
