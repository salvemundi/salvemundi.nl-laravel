...
@section('content')
    ...
    <div class="row widthFix adminOverlap center removeAutoMargin">
        ...
        <div class="col-auto col-md-10 col-sm-8">
            <h5 class="mt-5">Coupons kunnen niet worden verwijderd als ze al gebruikt zijn (er zit een transactie aan
                vastgekoppeld).</h5>
                    ...
                    <tbody>
                        @foreach ($coupons as $coupon)
                            <tr id="tr-id-3" class="tr-class-2" data-title="bootstrap table">
                                ...
                                <td data-value="{{ $coupon->description }}">
                                    {{ Illuminate\Support\Str::limit($coupon->description, 100) }}</td>
                                ...
                                <td data-value="{{ $coupon->id }}">
                                    <form method="get" action="/admin/coupons/edit/{{ $coupon->id }}"><button
                                ...
                                @if ($coupon->transactions->count() > 1)
                                    <td data-value="{{ $coupon->id }}">
                                        <form method="post" action="/admin/coupons/delete/{{ $coupon->id }}">
                                            @csrf<button disabled class="btn btn-danger">Verwijderen</button></form>
                                    </td>
                                @else
                                    <td data-value="{{ $coupon->id }}">
                                        <form method="post" action="/admin/coupons/delete/{{ $coupon->id }}">
        ...
        <div id="contact" class="col-md-6">
            ...
            <form action="/admin/coupons/create" method="post">
                ...
                <div class="form-group">
                    ...
                    <input value="EUR" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}"
                        value="{{ old('name') }}" id="name" name="valuta" placeholder="Valuta...">
                </div>
