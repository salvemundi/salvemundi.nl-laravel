    ...
    <div class="row widthFix adminOverlap center removeAutoMargin">
        <div class="col-auto col-md-10 col-sm-8">
            ...
            <ul class="nav nav-tabs" id="myTab" role="tablist">
                <li class="nav-item">
                    <button class="nav-link tabber" id="contact-tab" data-bs-toggle="tab" data-bs-target="#inschrijvingen"
                        type="button" role="tab" aria-controls="contact" aria-selected="false"><i
                            class="fa fa-credit-card"></i> Betaald</button>

                </li>
                <li class="nav-item">
                    <button class="nav-link tabber" id="contact-tab" data-bs-toggle="tab" data-bs-target="#gegevens"
                        type="button" role="tab" aria-controls="contact" aria-selected="false"><i
                            class="fas fa-user"></i> Ingeschreven</button>
                </li>
            </ul>
                                ...
                                <tbody>
                                    @foreach ($introObjects as $user)
                                        <tr id="tr-id-3" class="tr-class-2" data-title="bootstrap table">
                                            ...
                                            <td data-value="{{ $user->payment->paymentStatus }}">
                                                {{ \App\Enums\paymentStatus::fromValue($user->payment->paymentStatus)->key }}
                                            </td>
                                            ...
                                            <td data-value="{{ $user->studentYear }}">
                                                {{ \App\Enums\IntroStudentYear::fromValue($user->studentYear)->key }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                ...
                <div class="introCheck center">
                    <form method="POST" action="intro/store">
                        ...
                        @elseif($introSetting->settingValue == 0)
                            <input class="inp-cbx" id="cbx" name="cbx" type="checkbox"
                                style="display: none" />
                        @endif
                    ...
                    <form method="POST" action="intro/storeConfirm">
                        ...
                        @if ($introConfirmSetting->settingValue == 1)
                            <input class="inp-cdx" id="cdx" name="cdx" type="checkbox" checked
                                style="display: none" />
                        @elseif($introConfirmSetting->settingValue == 0)
                            <input class="inp-cdx" id="cdx" name="cdx" type="checkbox"
                                style="display: none" />
                        @endif
                        ...
                        &nbsp;
                        <a href="{{ route('export_excel.excelBetaald') }}" class="btn btn-primary btn-sm">Export to
                            Excel</a>
                        <a href="{{ route('export_excel.excelIedereen') }}" class="btn btn-primary btn-sm">Export niet
                            betaalde to Excel</a>
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu">
                                <li><a href="mailto:?bcc={{ implode(',', $emailsFirstYear) }}" class="dropdown-item">Maak
                                        mail voor 1e jaars die niet betaald hebben</a></li>
                                <li><a href="mailto:?bcc={{ implode(',', $emailsSecondYear) }}"
                                        class="dropdown-item">Maak mail voor 2e jaars die niet betaald hebben</a></li>
                                <li>
                                    <hr class="dropdown-divider">
                                </li>
                                <li><a href="mailto:?bcc={{ implode(',', $emailPaid) }}" class="dropdown-item">Maak mail
                                        richting iedereen die betaald heeft</a></li>
                                <li><a href="mailto:?bcc={{ implode(',', $emailNonPaid) }}" class="dropdown-item">Maak
                                        mail richting iedereen die niet betaald heeft</a></li>
                                <li><a href="mailto:?bcc={{ implode(',', $allEmails) }}" class="dropdown-item">Maak mail
                                        richting iedereen</a></li>
                            </ul>
                        </div>
                    </form>
                </div>
