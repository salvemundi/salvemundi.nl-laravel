                    ...
                    <tbody>
                        @foreach ($sideJobBank as $skill)
                            <tr id="tr-id-3" class="tr-class-2" data-title="bootstrap table">
                                ...
                                <td data-value="{{ $skill->studyProfile }}">
                                    {{ \App\Enums\StudyProfile::coerce($skill->studyProfile)->description }}</td>
                                <td data-value="{{ $skill->description }}">{{ $skill->description }}</td>
                                <td data-value="{{ $skill->minSalaryEuroBruto }}">
                                    {{ $skill->minSalaryEuroBruto ?? 'Onderhandelbaar' }} @if (isset($skill->minSalaryEuroBruto) && isset($skill->maxSalaryEuroBruto))
                                        t/m
                                    @endif {{ $skill->maxSalaryEuroBruto ?? null }}</td>
                                <td data-value="{{ $skill->minAmountOfHoursPerWeek }}">
                                    {{ $skill->minAmountOfHoursPerWeek ?? 'Onderhandelbaar' }} @if (isset($skill->minAmountOfHoursPerWeek) && isset($skill->maxAmountOfHoursPerWeek))
                                        t/m
                                    @endif {{ $skill->maxAmountOfHoursPerWeek ?? null }}</td>
                                <td data-value="{{ $skill->id }}">
                                    <form method="post" action="/admin/bijbaanbank/edit">@csrf<input type="hidden"
            ...
            <div class="p-2">
                <form action="/admin/bijbaanbank/store" method="post" enctype="multipart/form-data">
                    ...
                    <div class="input-group mb-2">
                        ...
                        <input type="number" step=".01" name="minimumSalary" aria-label="Minimum"
                            class="form-control">
                        <input type="number" step=".01" name="maximumSalary" aria-label="Maximum"
                            class="form-control">
                    </div>

                    <div class="input-group">
                        <span class="input-group-text">Min / max aantal uren</span>
                        <input type="number" step=".1" name="minimumHours" aria-label="Minimum"
                            class="form-control">
                        <input type="number" step=".1" name="maximumHours" aria-label="Maximum"
                            class="form-control">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea type="textarea" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}"
                            name="description" placeholder="omschrijving bijbaan...">{{ old('description') }}</textarea>
                    </div>
