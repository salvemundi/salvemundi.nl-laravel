                    ...
                    <tbody>
                        @foreach ($activities as $activity)
                            <tr id="tr-id-3" class="tr-class-2" data-title="bootstrap table">
                                ...
                                <td data-value="{{ $activity->description }}">
                                    {{ Illuminate\Support\Str::limit($activity->description, 100) }}</td>
                                <td data-value="{{ $activity->formsLink }}">
                                    {{ Illuminate\Support\Str::limit($activity->formsLink, 20) }}</td>
                                <td data-value="{{ $activity->imgPath }}">{{ $activity->imgPath }}</td>
                                <td data-value="{{ $activity->membersOnly }}">
                                    {{ $converted_res = $activity->membersOnly ? 'Ja' : 'Nee' }}</td>
                                <td data-value="{{ $activity->tags }}">
                                    @foreach ($activity->tags as $tag)
                                        <span class="badge {{ $tag->colorClass }}"><i class="{{ $tag->icon }}"></i>
                                            {{ $tag->name }}</span>
                                    @endforeach
                                </td>
                                <td data-value="{{ $activity->id }}"><a class="btn btn-primary"
                                        href="/admin/activities/{{ $activity->id }}/edit">Bewerken</a></td>
                                <td data-value="{{ $activity->id }}">
                                    <form method="post" action="/admin/activities/delete">@csrf<input type="hidden"
                                            name="id" id="id" value="{{ $activity->id }}"><button
                                            type="submit" class="btn btn-danger">Verwijderen</button></form>
                                </td>
                                <td data-value="{{ $activity->id }}"><a
                                        href="/admin/activities/{{ $activity->id }}/signups"
                                        class="btn btn-primary">Inschrijvingen</a></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <form action="/admin/activities/store" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control{{ $errors->has('link') ? ' is-invalid' : '' }}" value="{{ old('link') }}"
                        id="link" name="link" placeholder="Forms link...">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" min="0"
                        class="form-control{{ $errors->has('limit') ? ' is-invalid' : '' }}" value="{{ old('limit') }}"
                        id="limit" name="limit" placeholder="Limiet aantal inschrijvingen...">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" min="0" step=".01"
                        ...
                        value="{{ old('price2') }}" id="price2" name="price2" placeholder="Prijs...">
                </div>
