                        ...
                        @if ($clubs != null)
                            @foreach ($clubs as $club)
                                <tr id="tr-id-3" class="tr-class-2" data-title="bootstrap table">
                                    ...
                                    <td data-value="{{ $club->description }}">
                                        {{ Illuminate\Support\Str::limit($club->description, 20) }}</td>
                                    ...
                                    <td data-value="{{ $club->id }}">
                                        <form method="post" action="/admin/clubs/edit">@csrf<input type="hidden"
                                                name="id" value="{{ $club->id }}"><button type="submit"
                                                class="btn btn-primary">Bewerken</button></form>
                                    </td>
                                    <td data-value="{{ $club->id }}">
                                        <form method="post" action="/admin/clubs/delete">@csrf<input type="hidden"
                                                name="id" value="{{ $club->id }}"><button type="submit"
        ...
        <div id="contact" class="col-auto col-lg-6 col-md-6 col-sm-8 mb-2">
            ...
            <form action="/admin/clubs/store" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control{{ $errors->has('clubName') ? ' is-invalid' : '' }}"
                        value="{{ old('clubName') }}" id="clubName" name="clubName" placeholder="Club naam...">
                </div>

                <div class="form-group">
                    <label for="voornaam">Bijnaam</label>
                    <input class="form-control{{ $errors->has('nickName') ? ' is-invalid' : '' }}"
                        value="{{ old('nickName') }}" id="nickName" name="nickName" placeholder="Bijnaam...">
                </div>

                <div class="form-group">
                    <label for="voornaam">Naam oprichter</label>
                    <input class="form-control{{ $errors->has('founderName') ? ' is-invalid' : '' }}"
                        value="{{ old('founderName') }}" id="founderName" name="founderName"
                        placeholder="Naam oprichter...">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control{{ $errors->has('whatsappLink') ? ' is-invalid' : '' }}"
                        value="{{ old('whatsappLink') }}" id="whatsappLink" name="whatsappLink"
                        placeholder="WhatsApp link...">
                </div>

                <div class="form-group">
                    <label for="voornaam">Discord link</label>
                    <input class="form-control{{ $errors->has('discordLink') ? ' is-invalid' : '' }}"
                        value="{{ old('discordLink') }}" id="discordLink" name="discordLink"
                        placeholder="Discord link...">
                </div>

                <div class="form-group">
                    <label for="voornaam">Andere link</label>
                    <input class="form-control{{ $errors->has('otherLink') ? ' is-invalid' : '' }}"
                        value="{{ old('otherLink') }}" id="otherLink" name="otherLink" placeholder="Andere link...">
                </div>
