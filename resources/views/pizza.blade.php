    ...
    <div class="overlap removeAutoMargin row pt-5">
        <div class="col-12 col-md-6 p-1 p-md-3 p-lg-5">
            ...
            <form action="/pizza/store" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control{{ $errors->has('type') ? ' is-invalid' : '' }}" value="{{ old('type') }}"
                        id="type" name="type" placeholder="Pizza...">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}"
                        value="{{ old('description') }}" id="description" name="description" placeholder="Notitie...">
                </div>
        ...
        <div class="col-12 col-md-6 p-1 p-md-3 p-lg-5">
            ...
            <div class="table-responsive pt-md-5">
                        ...
                        @if ($pizzas != null)
                            @foreach ($pizzas as $pizza)
                                <tr id="tr-id-3" class="tr-class-2" data-title="bootstrap table">
                                    ...
                                    <td data-field="size">{{ App\Enums\PizzaLocations::coerce($pizza->location)->key }}
                                    </td>
                                    <td data-field="user">
                                        {{ $pizza->user->insertion ? $pizza->user->FirstName . ' ' . $pizza->user->insertion . ' ' . $pizza->user->LastName : $pizza->user->FirstName . ' ' . $pizza->user->LastName }}
                                    </td>
                                    <td data-field="description">{{ $pizza->description }}</td>
                                    <td data-field="delete">
                                        @if (
                                            $pizza->user->id == Illuminate\Support\Facades\Auth::user()->id ||
                                                Illuminate\Support\Facades\Auth::user()->isAdmin())
                                            @include('include.deleteOwnPizzaModal', ['pizza' => $pizza])
                                            <button data-bs-toggle="modal" data-bs-target="#deletePizza{{ $pizza->id }}"
                ...
                <button data-bs-toggle="modal" data-bs-target="#deletePizzas" class="btn btn-danger mt-2">Verwijder
                    alles</button>
            </div>
        </div>
    </div>
